...
from Home.xlrd.xldate import xldate_as_tuple , XLDateError
from Home.xlrd.biffh import XL_CELL_DATE , XL_CELL_ERROR, error_text_from_code
import sys
import glob
...
def showable_cell_value(celltype, cellvalue, datemode):
    if celltype == XL_CELL_DATE:
        try:
            showval = xldate_as_tuple(cellvalue, datemode)
        except XLDateError:
            e1, e2 = sys.exc_info()[:2]
            showval = "%s:%s" % (e1.__name__, e2)
    elif celltype == XL_CELL_ERROR:
        showval = error_text_from_code.get(
